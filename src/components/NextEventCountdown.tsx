...
import { getEventTimezone, getTimezoneLabel, getZonedClock, isEventLive, timeToMinutes } from "@/lib/eventTiming";
...
export const NextEventCountdown:React.FC<Props>=({events,now,onSelectEvent})=>{
  ...
  return <section className="now-board" aria-label="Today and live schedule">
    ...
    {live?<button className="next-match next-match--live" onClick={()=>onSelectEvent(live.event)}>
      ...
      <div className="next-match__timing"><div className="live-callout"><span/>{clockLabel(live.startTime)}{live.endTime?` – ${clockLabel(live.endTime)}`:""} · {getTimezoneLabel(live.timezone,now)}</div><ArrowUpRight/></div>
      ...
      {upcoming.length>0&&<div className="quiet-schedule" aria-label="Upcoming today">{upcoming.map(item=><button key={item.event.id} onClick={()=>onSelectEvent(item.event)}><OrgLogo orgName={item.event.orgName} logoUrl={item.event.orgLogoUrl} size="sm"/><span><small>{clockLabel(item.startTime)} · {getTimezoneLabel(item.timezone,now)}</small><strong>{item.event.name}</strong></span><CalendarClock/></button>)}</div>}
    </div>}
  </section>;
};
