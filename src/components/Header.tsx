...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="sticky top-0 z-50 liquid-glass-header w-full">
      ...
      <div className="flex items-center justify-between px-3 sm:px-6 h-12 sm:h-13 gap-2 max-w-[1400px] mx-auto w-full">

        {/* Left: wordmark */}
        <div className="flex items-center gap-2 shrink-0">
          <div className="flex items-center gap-2 cursor-pointer active:scale-95 transition-transform" onClick={onToday}>
            ...
            <div className="flex flex-col leading-none">
              ...
              <span className="font-display font-semibold text-amber-400 text-[8px] sm:text-[8.5px] tracking-[0.2em] uppercase mt-0.5">BR Calendar</span>
            </div>
          </div>
          ...
          {availableRegions.length > 0 && (
            <>
              ...
              <div className="hidden md:flex items-center gap-1.5 liquid-glass-input rounded-lg px-2 py-0.5 text-xs">
        ...
        <div className="flex items-center gap-1 sm:gap-1.5 py-0.5 touch-manipulation">
          {/* Prev/Next */}
          <button
          ...
          {/* View mode toolbar */}
          <div className="flex items-center gap-0.5 p-0.5 rounded-xl bg-white/[0.02] border border-white/[0.05]">
            {TOOLBAR_VIEWS.map((v) => (
              <button
                key={v.id + v.label}
        ...
        <div className="flex items-center gap-1 shrink-0 touch-manipulation">
          ...
          <button onClick={onExport} className="toolbar-btn" title="Export Calendar" aria-label="Export Calendar"><Share2 className="w-3.5 h-3.5" /></button>
          <button onClick={onWalkthrough} className="toolbar-btn" title="Help & Walkthrough" aria-label="Help"><HelpCircle className="w-3.5 h-3.5" /></button>
        </div>
