...
export const FiltersBar:React.FC<Props>=(props)=>{
  ...
  useEffect(()=>{const close=(event:MouseEvent)=>{const target=event.target as Element;if(target.closest(".filter-more__panel"))return;if(root.current&&!root.current.contains(target))setOpen(false)};document.addEventListener("mousedown",close);return()=>document.removeEventListener("mousedown",close)},[]);
  ...
  const filterPanel=<><button type="button" className="filter-more__scrim" onClick={()=>setOpen(false)} aria-label="Close filters"/><div className="filter-more__panel">
    <header><div><strong>Filter events</strong><span>Combine regions, games and timing</span></div><div className="filter-more__actions">{active>0&&<button onClick={()=>{props.onGamesChange([]);props.onRegionsChange([]);props.onTimingChange("all")}}>Clear all</button>}<button className="filter-more__close" onClick={()=>setOpen(false)} aria-label="Close filters"><X/></button></div></header>
